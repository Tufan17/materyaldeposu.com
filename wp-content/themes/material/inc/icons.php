<?php
/**
 * Temanin ozgun ikon seti.
 *
 * Elementor'dan devralinan SVG'ler; boyut ve renk Tailwind siniflari ile
 * disaridan verilir (fill=currentColor, h-10 w-10 gibi).
 *
 * @package Material
 */
 ...
 * Bir ikonu basar.
 *
 * @param string $name  Ikon adi.
 * @param string $class SVG'ye eklenecek Tailwind siniflari.
 */
function material_icon( $name, $class = 'h-10 w-10' ) {
	$icons = material_icon_set();
	...
	printf(
		'<svg class="%s" viewBox="%s" fill="currentColor" aria-hidden="true">%s</svg>',
		esc_attr( $class ),
		esc_attr( $icons[ $name ]['box'] ),
		$icons[ $name ]['path'] // phpcs:ignore WordPress.Security.EscapeOutput -- sabit SVG isaretlemesi.
	);
}
